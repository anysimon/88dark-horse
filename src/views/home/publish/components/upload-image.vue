<template>
  <div class="upload">
      <div class="preview" @click="loadImg(1)">
        <img v-if="value" :src="value" class="avatar">
        <i v-else class="el-icon-plus avatar-uploader-icon"></i>
      </div>
    <!-- 弹窗 -->
    <el-dialog :visible.sync="centerDialogVisible" width="55%" center>
        <!-- 导航栏 -->
          <el-tabs v-model="activeName" type="card">
            <el-tab-pane label="素材库" name="first">
                <el-radio-group v-model="radio1" @change="loadImg(1)">
                    <el-radio-button label="all">全部</el-radio-button>
                    <el-radio-button label="active">收藏</el-radio-button>
                    </el-radio-group>
                    <el-row :gutter="20" style="text-align: center;margin-top: 20px">
                        <el-col
                            class="colImg"
                            :span="6" v-for="item in images" :key="item.id"
                            style="margin-bottom:20px"
                            @click.native="check = item"
                            >
                            <img
                            :src="item.url"
                            height="100px"
                            width="128px"
                            class="image"
                            >
                            <img :class="{ check: item === check }" style="display: none" src="data:image/png;base64," alt="">
                        </el-col>
                        </el-row>
                        <el-pagination
                            align="center"
                            style="margin-top: 15px"
                            :background= "true"
                            :page-size="8"
                            layout="prev, pager, next, jumper"
                            :total="total"
                            @current-change="pageTurn"
                            >
                            </el-pagination>
                </el-tab-pane>
            <el-tab-pane label="上传图片" name="second">
              <div style="" align="center">
                <img v-if="UploadImageUrl" :src="UploadImageUrl" class="avatar">
                <i v-else class="el-icon-picture"></i>
            </div>
              <el-upload
                class="upload-demo"
                action="https://jsonplaceholder.typicode.com/posts/"
                :http-request="onChange"
                >
                <span style="margin-left: 50px; margin-right: 10px">用户图片</span>
                <el-button size="small" type="primary">点击选择图片</el-button>
              </el-upload>
             <el-button
              type="primary"
              size="small"
              style="margin-left: 116px; margin-top: 10px" @click="onImages">
              开始上传
             </el-button>
            </el-tab-pane>
        </el-tabs>
        <!-- 导航栏 -->
      <span slot="footer" class="dialog-footer">
        <el-button @click="centerDialogVisible = false">取 消</el-button>
        <el-button type="primary" @click="onConfirm">确 定</el-button>
      </span>
    </el-dialog>
    <!-- /弹窗 -->
  </div>
</template>

<script>
export default {
  name: 'uploadImg',
  props: {
    value: {
      type: String
    }
  },
  data () {
    return {
      images: [],
      centerDialogVisible: false,
      activeName: 'first', // 素材库/上传图片
      radio1: 'all', // 全部/收藏
      total: 8, // 每页显示项目数
      // page: null, // 页码数
      imageUrl: null, // 发布文章界面的图片
      file: '',
      UploadImageUrl: null, // 点击开始上传之后的图片地址
      check: null // 图片类名判断
    }
  },
  methods: {
    // 点击加载图片
    loadImg (page) {
      this.centerDialogVisible = true
      this.$axios({
        url: '/user/images',
        params: {
          page,
          per_page: 8,
          collect: this.radio1 !== 'all'
        }
      }).then(res => {
        this.images = res.data.data.results
        this.total = res.data.data.total_count
      }).catch(err => {
        console.log(err)
      })
    },
    // 翻页
    pageTurn (page) {
      // this.page = page
      this.loadImg(page)
    },
    onChange (config) {
      this.file = config.file
    },
    onImages () {
      const formdata = new FormData()
      formdata.append('image', this.file)
      this.$axios({
        method: 'POST',
        url: '/user/images',
        data: formdata
      }).then(res => {
        this.UploadImageUrl = res.data.data.url
      })
    },
    // 点击确定
    onConfirm () {
      if (this.activeName === 'first') {
        if (this.check) {
          this.$emit('input', this.check.url)
        }
      } else if (this.activeName === 'second') {
        if (this.UploadImageUrl) {
          this.$emit('input', this.UploadImageUrl)
        }
      }
      this.centerDialogVisible = false
    }
  }
}
</script>

<style lang="less" secoped>
.image {
    border-radius: 5px;
}
  .avatar-uploader-icon {
    font-size: 28px;
    color: #8c939d;
    line-height: 178px !important;
    text-align: center;
    border: 1px solid #ccc;
  }
  .avatar,.avatar-uploader-icon {
    margin-bottom: 20px;
    margin-left: 36px;
    width: 178px;
    height: 178px;
  }
  .el-icon-picture {
    width: 178px;
    height: 178px;
    line-height: 178px !important;
    font-size:50px;
  }
  .el-upload-list--text {
    width: 300px;
    margin-left: 116px !important;
  }
  .colImg {
    position: relative;
  }
  .check {
    display: block !important;
    width: 60px;
    height: 60px;
    position: absolute;
    bottom: 0;
    left: 0;
    margin-left: 35%;
    margin-bottom: 15%;
  }
</style>
